...
class SimpleFlowConverter:
    ...
    def __init__(self, simple_flow: list):
        ...
        self.nodes = {}  # id -> node
        self.links = []  # Link 列表
        self.variables = []  # Variable 列表
        self.template_name = ""  # 模板名称
        self.id_mapping = {}  # 旧ID -> 新ID 映射
        self._parse_input()
    ...
    def _wrap_data_value(self, value):
        ...
        :return: 标准格式的对象
        """
        # 如果已经是标准格式（包含 hook 和 value 字段），直接返回
    ...
    def _get_latest_component_version(self, code):
        ...
        try:
            # 查询该 code 的所有版本
            versions = ComponentModel.objects.filter(
                code=code, status=1
            ...
            logger.info("_get_latest_component_version: code={}, versions={}, latest={}".format(
    ...
    def convert(self) -> dict:
        ...
        for link in self.links:
            ...
            # 记录 source -> (flow_id, target, 原始target_id) 映射
            if source in source_to_flows:
                original_target = link["target"]  # 保留原始 target ID 用于匹配 conditions
    ...
    def _build_activity(self, node, incoming, outgoing) -> dict:
        ...
        """
        # outgoing 处理：单个出边用字符串，多个用列表
        outgoing_value = outgoing[0] if len(outgoing) == 1 else outgoing

        # 标准化 component data，确保所有字段值都是标准格式
    ...
    def _build_gateway(self, node, incoming, outgoing, outgoing_flows=None) -> dict:
        ...
        # ConvergeGateway 的 outgoing 是字符串，其他网关是数组
        ...
        if node_type == "ExclusiveGateway":
            gateway["conditions"] = self._build_gateway_conditions(
                node.get("conditions", {}), outgoing_flows
            )
    ...
    def _build_gateway_conditions(self, raw_conditions, outgoing_flows) -> dict:
        ...
        if not raw_conditions or not outgoing_flows:
            # 如果没有 conditions，为每个出边创建默认 condition
            conditions = {}
        ...
        # 兼容列表格式的 conditions
        # 列表格式: [{"target": "node1", "name": "条件1", "expression": "..."}, ...]
        # 字典格式: {"node1": {"name": "条件1", "expression": "..."}, ...}
        if isinstance(raw_conditions, list):
            condition_items = []
            for cond in raw_conditions:
                if isinstance(cond, dict):
                    # 尝试从 target 字段获取目标节点 ID
        ...
        for cond_key, cond_value in condition_items:
            ...
            if not flow_id:
                # 如果 cond_key 不是目标节点 ID，可能是临时 ID
                # 尝试按顺序匹配
            ...
            if flow_id:
                ...
                # 获取表达式，支持多种字段名
                expression = (
                    cond_value.get("evaluate") or
                    cond_value.get("expression") or
                    cond_value.get("expr") or
                    "True"
                )
                conditions[flow_id] = {
                    "evaluate": expression,
